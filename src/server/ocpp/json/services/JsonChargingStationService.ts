...
import { CallbackHandler } from '../../../../callback/CallbackHandler';
import { CallbackTrigger } from '../../../../callback/CallbackTriggerService';
...
export default class JsonChargingStationService {
  ...
  private async handle(command: Command, headers: OCPPHeader, payload) {
    try {
      this.callbackTrigger.submitCallBack(headers.tenantID, `${command}` as EventTypes, payload);
